...
class GMProto:
    ...
    def __format(self, prefix: str) -> str:
        ...
        next_prefix = prefix if self._field_key is None else key + "."

        lines = [f"{key}: {self._total}/{self._bound} {dict(self._times)}"]
    ...
    def _finish(self) -> None:
        self._total = sum(size * count for size, count in self._times.items())
    ...
    def extract_proto(cls, key: str) -> GMProto:
        messages = [template.value for template in get_templates(key).values()]
