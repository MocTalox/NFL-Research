...
from core.gm_holoholo import HoloPokemonMove, HoloCombatType
...
@dataclass(frozen=True)
class BonusEffect:
    attack_defense_bonus: AttackDefenseBonus

    @classmethod
    def from_message(cls, msg: Message) -> BonusEffect:
        return cls(
            attack_defense_bonus=msg.get_object("attackDefenseBonus", AttackDefenseBonus.from_message),
        )
...
class Attributes:
    ...
    def from_message(cls, msg: Message) -> Attributes:
        return cls(
            ...
            defense_multiplier=msg.get_float("defenseMultiplier"),
        )
