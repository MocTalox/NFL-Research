...
def get_tgr_stats(
    poke: PokemonSettings, cpm: float, rank: HoloCharacterCategory, iv_atk: int, iv_def: int, iv_sta: int
) -> tuple[float, float, float]:
...
def get_tgr_cp(poke: PokemonSettings, cpm: float, rank: HoloCharacterCategory, iv_atk: int, iv_def: int, iv_sta: int) -> int:
    return _get_cp(*get_tgr_stats(poke, cpm, rank, iv_atk, iv_def, iv_sta))

def get_tgr_hp(poke: PokemonSettings, cpm: float, rank: HoloCharacterCategory, iv_sta: int) -> int:
